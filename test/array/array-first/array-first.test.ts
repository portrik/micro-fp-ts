...
describe('array-first', function() {
	it('should return none for empty array:', () => {
		expect(array.arrayFirst([])).toEqual(none);
	});

	it('should return none for size under 1', () => {
		expect(array.arrayFirst([1, 2, 3, 4], 0)).toEqual(none);
	});

	it('should return the first element in the array:', () => {
		expect(array.arrayFirst(['a', 'b', 'c', 'd', 'e', 'f'])).toEqual(some('a'));
		expect(array.arrayFirst(['a', 'b', 'c', 'd', 'e', 'f'], 1)).toEqual(some('a'));
	});

	it('should return the first n elements of the array:', () => {
		expect(array.arrayFirst(['a', 'b', 'c', 'd', 'e', 'f'], 3)).toEqual(some(['a', 'b', 'c']));
	});

	it('should return the first n elements of the array even for size larger than array length', () => {
		expect(array.arrayFirst(['a', 'b', 'c', 'd'], 10)).toEqual(some(['a', 'b', 'c', 'd']));
	});
});
